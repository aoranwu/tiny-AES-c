...
void run_with_len(struct AES_ctx ctx, uint8_t in[][16], int len)
{
    for (int i = 0; i < len; i++)
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2)
    {
        printf("Usage: ./get_roofline FLOPS_PER_BYTE BLOCK_SIZE(unit:16B)");
        exit(1);
    }
    ...
    uint8_t in32B = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a,
    ...
    long long op_per_byte = atoll(argv[1]);
    float alpha = 0.5;
    float beta = 0.5;
    float A[] = {3.5, 5.36, 6.32};
    ...
    printf("flops_per_byte:%lld\n", op_per_byte*2);
    // printf("Time elapsed:%f s\n", msec/1000);
    elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000.0;    // sec to ms
    elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0; // us to ms
    // printf("%f s.\n", elapsedTime/1000);
    float flops_per_second = (1.0 * op_per_byte*2) / (1.0 * (elapsedTime / 1000));
